...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    val qid2question = postings.filter(_.postingType == 1).map(x => (x.id, x))
    val qid2answer = postings.filter(x => (x.postingType == 2)).map(x => (x.parentId.get, x))
    qid2question.join(qid2answer).groupByKey()
  }
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.map{case (id, posts) => (posts.head._1, answerHighScore(posts.map(_._2).toArray))}
  }
...
//    val partitioned = meanOfPoints.partitionBy(tunedPartitioner).cache
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      val domainIdx = vs.groupBy(_._1).map{case (k, v) => (k, v.size)}.maxBy(_._2)._1
      val langLabel: String   = langs(domainIdx/50000) // most common language in the cluster
      val langPercent: Double = (vs.filter(_._1 == domainIdx).size / vs.size.toDouble)*100 // percent of the questions in the most common language
      val clusterSize: Int    = vs.size.toInt
      val medianScore: Int    = findMedian(vs.map(_._2).toList.sorted)
