...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[4]").setAppName("WikipediaRankingApp")
...
//    rdd.aggregate(0)(
//        (sum, x) => {
//          val num = if(x.mentionsLanguage(lang)) 1 else 0
//          sum + num
//        },
//        _+_)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.map(r => (r, r.text.split(' ').intersect(langs).distinct))
       .flatMap(x => x._2.map(y => (y, x._1)))
       .groupBy(_._1)
       .mapValues(_.map(x => x._2))
  }
